...
class TradingOrchestrator:
    ...
    def collect_news(self) -> List[Dict]:
        ...
        """
        news_list = (
            multi_source_scraper.fetch_all_news(max_total_articles=MAX_NEWS_ARTICLES)
            or []
        )
    ...
    def _store_news_articles(self, news_list: List[Dict]) -> None:
        ...
        if stored_count < len(news_list):
            logger.info(
                f"Skipped {len(news_list) - stored_count} duplicate news articles"
            )
    ...
    def _analyze_and_record_trade(self, symbol: str, decision: Dict) -> None:
        ...
        try:
            ...
            logger.info(
                LOG_AI_ANALYSIS_RECORDED.format(
    ...
    def _create_trade_analysis(
        self, symbol: str, decision: Dict, market_dict: Dict
    ) -> TradeAnalysis:
    ...
    def _record_enhanced_trade(
        ...
        if action in ["buy", "buy_more"]:
            ...
            profit_percent, avg_buy_price, used_buy_ids = (
                self.data_store.calculate_sell_profit(symbol, current_price, quantity)
            )
            ...
            logger.info(
                f"📊 {symbol} {action}: {profit_percent:.2f}% return (avg buy: {avg_buy_price:,.0f}원)"
            )
    ...
    def _execute_trading_cycle(self, cycle_start_time: float) -> None:
        ...
        decisions = self._get_ai_decisions_with_safety(
            news_list, market_data, portfolio
        )
    ...
    def _collect_and_validate_data(
        self,
    ) -> Tuple[List[Dict], List[str], Dict[str, Dict], Dict]:
    ...
    def _get_ai_decisions_with_safety(
        self, news_list: List[Dict], market_data: Dict[str, Dict], portfolio: Dict
    ) -> Dict[str, Dict]:
        ...
        return self._apply_adaptive_risk_management(
    ...
    def _apply_trade_history_insights(self, decisions: Dict[str, Dict]) -> None:
        ...
        for symbol, decision in decisions.items():
            ...
            if self.trade_analyzer.should_avoid_symbol(symbol):
                logger.warning(
                    f"⚠️ {symbol} has poor history ({success_rate}%), adjusting confidence"
                )
    ...
    def _apply_adaptive_risk_management(
        ...
        for symbol, decision in validated_decisions.items():
            ...
            if action in [ACTION_BUY, ACTION_BUY_MORE]:
                decision = self._apply_position_sizing(
                    decision, portfolio, market_data.get(symbol, {})
                )

            # Apply sell percentage calculation for partial sells
            elif action == ACTION_PARTIAL_SELL:
                decision = self._apply_sell_sizing(
                    decision, portfolio, market_data.get(symbol, {})
                )

            # Add stop-loss recommendations for all positions
            if action in [ACTION_BUY, ACTION_BUY_MORE, ACTION_HOLD]:
                decision = self._add_stop_loss_recommendation(
                    decision, market_data.get(symbol, {})
                )
    ...
    def _apply_position_sizing(
        self, decision: Dict, portfolio: Dict, market_data: Dict
    ) -> Dict:
        ...
        decision["recommended_amount_krw"] = sizing_result.get(
            "recommended_size", MIN_VIABLE_TRADE_AMOUNT
        )
    ...
    def _apply_sell_sizing(
        ...
        try:
            ...
            decision["sell_reason"] = (
                f"AI-optimized partial sell: {sell_percentage:.1%}"
            )
...
"""
    ...
    def _add_stop_loss_recommendation(self, decision: Dict, market_data: Dict) -> Dict:
...
"""
    ...
    def _apply_circuit_breaker_limits(self, decision: Dict, portfolio: Dict) -> Dict:
        ...
        recent_decisions = self.data_store.get_recent_trades(
            limit=HEALTH_CHECK_TRADE_LIMIT_CIRCUIT, hours=HEALTH_CHECK_TRADE_HOURS
        )
        ...
        if circuit_result.get("circuit_breaker_triggered", False):
            ...
            if circuit_result.get("adjusted_size", 0) < MIN_VIABLE_TRADE_AMOUNT:
                ...
                decision["reason"] = (
                    f"Circuit breaker: {circuit_result.get('reason', 'Trade size too small')}"
                )

        return decision

    def _apply_risk_management(
        self, decisions: Dict[str, Dict], portfolio: Dict, market_data: Dict[str, Dict]
    ) -> Dict[str, Dict]:
    ...
    def _monitor_portfolio_risks(
        self, portfolio: Dict, market_data: Dict[str, Dict]
    ) -> None:
        ...
        recent_trades = self.data_store.get_recent_trades(
            limit=HEALTH_CHECK_RECENT_TRADES_LIMIT, hours=HEALTH_CHECK_TRADE_HOURS
        )
        ...
        if risk_assessment.get("overall_risk") in [
            RISK_LEVEL_HIGH,
            RISK_LEVEL_CRITICAL,
        ]:
            logger.warning(
                LOG_HIGH_RISK_DETECTED.format(risk_assessment.get("overall_risk"))
            )
            logger.info(
                LOG_AI_RISK_ASSESSMENT.format(
    ...
    def _apply_stop_loss_triggers(
        ...
        """
        stop_loss_triggers = self.risk_monitor.check_stop_loss_triggers(
    ...
    def _execute_trades_with_analysis(
        self, decisions: Dict[str, Dict], market_data: Dict[str, Dict], portfolio: Dict
    ) -> None:
    ...
    def _perform_post_trade_analysis(
        ...
        for symbol, decision in decisions.items():
            if self._is_stop_loss_decision(decision):
                self._analyze_stop_loss_decision(
                    symbol, decision, market_data, portfolio
                )
    ...
    def _is_stop_loss_decision(self, decision: Dict) -> bool:
        ...
        """
        return bool(
            decision.get("stop_loss_trigger")
            and decision.get("action") in [ACTION_SELL_ALL, ACTION_PARTIAL_SELL]
        )

    def _analyze_stop_loss_decision(
        self, symbol: str, decision: Dict, market_data: Dict[str, Dict], portfolio: Dict
    ) -> None:
        ...
        try:
            ...
            if not detailed:
                ...
                coin_count = sum(
                    ...
                    if k != "KRW" and v.get("balance", 0) > 0
                )

                logger.info(
                    f"📊 Portfolio: {total_value:,.0f}원 ({total_profit_pct:+.2f}%) | "
                    f"{coin_count} coins | KRW: {portfolio.get('available_krw', 0):,.0f}원"
                )
                return
            ...
            logger.info(
                f"📈 Total P&L: {total_profit:+,.0f}원 ({total_profit_pct:+.2f}%)"
            )

            # Individual coin status
            assets = portfolio.get("assets", {})
            coin_count = sum(
                1 for k, v in assets.items() if k != "KRW" and v.get("balance", 0) > 0
            )
            ...
            if coin_count > 0:
                ...
                for symbol, asset in sorted_assets:
                    ...
                    emoji = (
                        ...
                        else "🔴" if profit_loss_pct < 0 else "⚪"
                    )
            ...
            # Available KRW
            krw_balance = portfolio.get("available_krw", 0)
