...
LOG_BUY_ERROR_MIN_ORDER = (
    "[BUY ERROR] {}: Amount {:.0f} KRW below {:.0f} KRW. Skipping."
)
...
class UpbitTrader:
    ...
    def _validate_buy_order(self, symbol: str, krw_amount: float) -> bool:
        ...
        if krw_amount < MIN_ORDER_KRW:
            logger.error(
                LOG_BUY_ERROR_MIN_ORDER.format(symbol, krw_amount, MIN_ORDER_KRW)
            )
            return False
    ...
    def _record_and_log_purchase(
        self, symbol: str, krw_amount: float, price: float
    ) -> None:
    ...
    def execute_trade(
        ...
        logger.info(
            LOG_TRADE_EXECUTION.format(
                action, symbol, current_date, current_time, reason
            )
        )
    ...
    def _validate_buy_more_action(
        self, symbol: str, action: str, decision_data: Optional[Dict[str, Any]]
    ) -> str:
    ...
    def _dispatch_trade_action(
        ...
        if action == ACTION_PARTIAL_SELL:
            sell_percentage = (
                decision_data.get("sell_percentage") if decision_data else None
            )
    ...
    def _buy_average_down(self, symbol: str, avg_analysis: Dict[str, Any]) -> bool:
        ...
        # Check we have enough KRW before averaging down, using the trader's own
        # portfolio status (the previous PortfolioManager(self.client) path referenced
        # attributes/classes that do not exist and always failed).
        available_krw = self.get_portfolio_status().get("available_krw", 0)
        if available_krw < amount:
            logger.error(
                f"Insufficient KRW for averaging down {symbol}. "
                f"Required: {amount:,.0f} KRW, Available: {available_krw:,.0f} KRW"
            )
            return False
    ...
    def _calculate_averaging_amount(self, avg_analysis: Dict[str, Any]) -> float:
        ...
        """
        confidence = float(avg_analysis.get("confidence", DEFAULT_CONFIDENCE))
        confidence = max(
            MIN_AVERAGING_CONFIDENCE, min(confidence, MAX_AVERAGING_CONFIDENCE)
        )
    ...
    def _execute_partial_sell(
        self, symbol: str, sell_percentage: Optional[float] = None
    ) -> bool:
        ...
        percentage = (
            sell_percentage if sell_percentage is not None else PARTIAL_SELL_RATIO
        )
        ...
        logger.info(
            f"💰 Executing partial sell for {symbol}: {amount:.6f} ({percentage:.1%} of holdings)"
        )
    ...
    def _get_symbol_balance(self, symbol: str) -> Optional[float]:
        ...
        return float(portfolio["assets"][symbol]["balance"])
