...
class MultiSourceNewsScraper:
    ...
    def fetch_all_news(
        self, max_total_articles: int = DEFAULT_MAX_ARTICLES
    ) -> List[Dict[str, Any]]:
        """Fetch news from all available sources.

        Uses the enhanced RSS aggregator as the primary (and only) source.
        ...
        try:
            ...
            if not rss_articles:
                logger.warning("⚠️ 뉴스 수집 결과 없음")
                return []

            sufficient = (
                len(rss_articles)
                >= max_total_articles // RSS_SUFFICIENT_THRESHOLD_DIVISOR
            )
            if sufficient:
                logger.info("✅ RSS 수집 성공: %d개 기사", len(rss_articles))
            else:
                logger.warning("⚠️ RSS 수집 부족: %d개 기사", len(rss_articles))
            return rss_articles
