...
class EnhancedMarketDataCollector:
    ...
    def _get_price_data(self, market_code: str, current_price: float) -> Dict[str, Any]:
        """Get comprehensive price analysis data."""
        try:
            # Get different timeframe data
            df_1h = pyupbit.get_ohlcv(
                market_code, interval="minute60", count=24
            )  # 24 hours
            ...
            if len(df_1d) >= 31:
                ...
                price_30d_change = (
                    (current_price - price_30d_ago) / price_30d_ago
                ) * 100
    ...
    def _get_volume_data(self, market_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            vwap_7d = (
                self._calculate_vwap(df_1d.iloc[-7:]) if len(df_1d) >= 7 else vwap_24h
            )
    ...
    def _get_technical_indicators(self, market_code: str) -> Dict[str, Any]:
        ...
        try:
            df = pyupbit.get_ohlcv(
                market_code, interval="day", count=200
            )  # Need more data for EMA 200
            if df is None or len(df) < 30:
                logger.warning(
                    f"Insufficient data for technical indicators: {market_code}, got {len(df) if df is not None else 0} rows"
                )
            ...
            ema_9 = (
                float(closes.ewm(span=9).mean().iloc[-1]) if len(closes) >= 9 else 0.0
            )
            ema_21 = (
                float(closes.ewm(span=21).mean().iloc[-1]) if len(closes) >= 21 else 0.0
            )
            ema_50 = (
                float(closes.ewm(span=50).mean().iloc[-1]) if len(closes) >= 50 else 0.0
            )
            ema_200 = (
                float(closes.ewm(span=200).mean().iloc[-1])
                if len(closes) >= 200
                else 0.0
            )
            ...
            if len(highs) > 0 and len(lows) > 0 and len(closes) > 0:
                pivot_points = self._calculate_pivot_points(
                    highs.iloc[-1], lows.iloc[-1], closes.iloc[-1]
                )
            else:
                pivot_points = {
                    "pivot_point": 0.0,
    ...
    def _calculate_stochastic(
        ...
        # Guard flat windows (high == low) so the result is NaN (handled below) not inf.
    ...
    def _get_support_resistance_levels(self, market_code: str) -> Dict[str, Any]:
    ...
    def _get_volatility_metrics(self, market_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            volatility_1d = (
                float(abs(returns.iloc[-1]) * 100) if len(returns) >= 1 else 0.0
            )
            volatility_7d = (
                float(returns.iloc[-7:].std() * 100) if len(returns) >= 7 else 0.0
            )
            volatility_30d = float(returns.std() * 100) if len(returns) >= 30 else 0.0

            # returns is already dropna()'d and length-guarded above, so the
            # volatility values are always finite floats here.
            return {
    ...
    def _get_correlation_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            correlation_7d = (
                float(symbol_returns.iloc[-7:].corr(btc_returns.iloc[-7:]))
                if min_len >= 7
                else 0.0
            )
            correlation_30d = (
                float(symbol_returns.corr(btc_returns)) if min_len >= 30 else 0.0
            )
    ...
    def _calculate_adx(
        ...
        # Guard no-movement windows (atr == 0) to avoid inf in the DI ratios.
        ...
        adx_val, pos_val, neg_val = adx.iloc[-1], pos_di.iloc[-1], neg_di.iloc[-1]
        return (
            0.0 if pd.isna(adx_val) else float(adx_val),
            0.0 if pd.isna(pos_val) else float(pos_val),
            0.0 if pd.isna(neg_val) else float(neg_val),
        )
    ...
    def _calculate_ichimoku(
        ...
        return {
            ...
            "ichimoku_chikou": round(
                float(chikou_span.iloc[-27]) if len(chikou_span) > 26 else 0.0, 2
            ),
        }
    ...
    def _calculate_obv(
        ...
        # Down days flip volume sign, flat days contribute zero; up/first days stay positive.
        # Use where/mask (not in-place masked assignment) for pandas Copy-on-Write safety.
        volume_direction = volumes.where(~(price_diff < 0), -volumes)
        volume_direction = volume_direction.mask(price_diff == 0, 0.0)
    ...
    def _calculate_mfi(
        ...
        positive_mf = positive_flow.rolling(window=period).sum().iloc[-1]
        negative_mf = negative_flow.rolling(window=period).sum().iloc[-1]

        if pd.isna(positive_mf) or pd.isna(negative_mf):
            return 50.0
        if negative_mf == 0:
            # No down-flow in the window: fully bullish (100) or flat (neutral 50).
    ...
    def _calculate_williams_r(
        ...
        price_range = (highest_high - lowest_low).replace(0, np.nan)
        williams_r = -100 * ((highest_high - closes) / price_range)

        result = williams_r.iloc[-1]
