...
class DataStore:
    ...
    def record_ai_analysis(self, analysis: AIAnalysisResult) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Recorded AI analysis with {len(analysis.decisions)} decisions"
            )
            return True
    ...
    def record_trade_analysis(self, analysis: TradeAnalysis) -> None:
    ...
    def calculate_sell_profit(
        self, symbol: str, sell_price: float, sell_quantity: float
        ...
        """
        try:
            # Hold the lock across the whole read-modify-write so concurrent sells
            # cannot consume the same open lots twice.
            with self.lock:
                ...
                if total_quantity < sell_quantity:
                    logger.warning(
                        f"Sell quantity {sell_quantity:.8f} for {symbol} exceeds tracked "
                        f"open inventory {total_quantity:.8f}; profit computed on the filled portion."
                    )
...
def get_data_store() -> DataStore:
    """Return a lazily-created process-wide DataStore (avoids DB I/O at import time)."""
...
def get_latest_portfolio_status() -> Optional[Dict[str, Any]]:
    """Return the most recently recorded portfolio snapshot, or None if none exists.

    Reads the latest ai_analysis row so risk components can estimate a recent
    portfolio value without holding a DataStore reference.
    """
    ...
    try:
        ...
        logger.error(f"Failed to get latest portfolio status: {exc}")
        return None
    if not row:
        return None
    return {"total_balance": row[0], "timestamp": row[1]}
