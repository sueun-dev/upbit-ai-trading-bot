...
class MultiAIValidator:
    ...
    def cross_validate_multiple_decisions(
        ...
        for symbol, decision in decisions.items():
            try:
                ...
                final_decision = self._aggregate_validations(
                    decision, validations, symbol, portfolio
                )
    ...
    def _validate_aggressive(
        ...
        prompt = self._create_aggressive_prompt(
            symbol, decision, market_data, news_list
        )
    ...
    def _aggregate_validations(
        ...
        self._apply_validation_rules(
            final_decision, original_decision, approval_rate, symbol, portfolio
        )
    ...
    def _calculate_average_confidence(
        ...
        """
        confidences = [
            v.get("confidence", DEFAULT_CONFIDENCE) for v in validations.values()
        ]
        return float(np.mean(confidences)) if confidences else DEFAULT_CONFIDENCE
    ...
    def _calculate_consensus_factor(
        ...
        if not confidences:
            return 1.0
        # Clamp to [0, 1] so a high-variance panel cannot drive confidence negative.
        return float(max(0.0, min(1.0, 1 - np.std(confidences))))
    ...
    def _apply_validation_rules(
        ...
        if approval_rate < MAJORITY_REJECTION_THRESHOLD:
            ...
            if original_action in BUY_ACTIONS:
                # If rejected buy action, change to skip (not hold)
                final_decision["action"] = "skip"
                final_decision["reason"] = (
                    f"Multi-AI validation rejected buy: {approval_rate:.0%} approval"
                )
            elif original_action in SELL_ACTIONS:
                # For sells, be more cautious about overriding
                if approval_rate < STRONG_REJECTION_THRESHOLD:
                    final_decision["action"] = HOLD_ACTION
                    final_decision["reason"] = (
                        f"Multi-AI strongly rejected sell: {approval_rate:.0%} approval"
                    )

    def _add_validation_metadata(
        self, decision: Dict[str, Any], approval_rate: float, consensus_factor: float
    ) -> None:
    ...
    def _create_technical_prompt(
        self, symbol: str, decision: Dict[str, Any], market_data: Dict[str, Any]
    ) -> str:
