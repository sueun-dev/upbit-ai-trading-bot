...
VALID_TRADING_ACTIONS = ["buy", "sell_all", "partial_sell", "hold", "buy_more"]
...
class AIAnalyzer:
    ...
    def extract_symbols_from_news(self, news_list: List[Dict[str, Any]]) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"Extracted {len(valid_symbols)} symbols from {len(news_list)} news articles"
            )
        ...
        try:
            ...
            for symbol, decision in decisions.items():
                ...
                if action == "hold" and symbol not in held_assets:
                    logger.error(
                        f"AI ERROR: Assigned 'hold' to {symbol} which is not in portfolio. This should be 'buy' or 'skip'!"
                    )
            ...
            if not validated_decisions:
                logger.warning(
                    f"No valid trading decisions extracted from AI response: {result}"
                )
    ...
    def _parse_symbol_extraction_response(self, result: Any) -> List[str]:
    ...
    def _parse_symbols_from_string(self, result: str) -> List[str]:
        ...
        try:
            ...
            pass
        # Not a JSON list; extract symbols using regex.
        return re.findall(r"\b[A-Z]{2,10}\b", result)
