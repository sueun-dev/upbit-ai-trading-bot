...
class AILearningSystem:
    ...
    def analyze_historical_trades(self, days_back: int = 30) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Generated {len(insights)} learning insights from {len(trades)} trades"
            )
            return insights
    ...
    def _analyze_success_rate(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        sell_trades = [
            t
            for t in trades
            if t.get("action") in ["partial_sell", "sell_all"]
        ...
        avg_profit = (
            sum(t.get("actual_return", 0) for t in successful) / len(successful)
            if successful
            else 0
        )
        avg_loss = (
            sum(t.get("actual_return", 0) for t in failed) / len(failed)
            if failed
            else 0
        )
    ...
    def _analyze_failed_trades(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if not sell_trades:
            return {
                ...
                "metrics": {"total_failures": 0, "failure_rate": 0.0},
                "common_reasons": {},
        ...
        action_failures: defaultdict[str, int] = defaultdict(int)
        ...
        return {
            ...
            "recommendation": recommendations.get(
                most_common[0], "Conduct detailed analysis of failure patterns"
            ),
            "priority": "high",
        }
    ...
    def should_avoid_symbol(self, symbol: str, threshold: float = 40.0) -> bool:
        ...
        return (
            success_rate > 0 and success_rate < threshold
        )  # Only avoid if we have history AND it's bad
            ...
            if lesson.get("type") == "failure_analysis":
                ...
                if failure_rate > 40:  # High failure rate
                    ...
                    if most_common == "stop_loss":
                        warnings.append(
                            "⚠️ High stop-loss triggers (adjust thresholds)"
                        )
    ...
    def get_trading_insights_for_symbol(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Average only over rows that actually carry a return (keep legitimate
                # 0.0 values; divide by the count of those rows, not the full count).
                returns = [t[2] for t in recent_trades if t[2] is not None]
                patterns = {
                    "recent_performance": (
                        "improving"
                        if recent_trades and recent_trades[0][1]
                        else "declining"
                    ),
                    "common_action": (
                        "partial_sell"
                        if any(t[0] == "partial_sell" for t in recent_trades)
                        else "sell_all"
                    ),
                    "avg_recent_return": sum(returns) / len(returns) if returns else 0,
                }
    ...
    def _generate_symbol_recommendation(
        self, symbol: str, patterns: Dict[str, Any]
    ) -> str:
