...
class RiskMonitor:
    ...
    def check_stop_loss_triggers(
        self, portfolio: Dict[str, Any], market_data: Dict[str, Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for symbol, asset in assets.items():
            ...
            profit_loss_pct = (
                float(asset.get("profit_loss_percentage", 0)) / PERCENTAGE_DIVISOR
            )

            # Check stop-loss thresholds
            trigger = self._check_position_stop_loss(symbol, asset, profit_loss_pct)
    ...
    def _check_position_stop_loss(
        self, symbol: str, asset: Dict[str, Any], profit_loss_pct: float
    ) -> Optional[Dict[str, Any]]:
        """Check if a position breaches an emergency or regular stop-loss threshold.

        Args:
            symbol: Asset symbol.
            asset: Asset information.
            profit_loss_pct: Profit/loss as a ratio (e.g. -0.16 for -16%).
    ...
    def _create_stop_loss_trigger(
        self, symbol: str, loss_pct: float, trigger_type: str, action: str, reason: str
    ) -> Dict[str, Any]:
    ...
    def _analyze_single_position_risk(
        self, asset: Dict[str, Any], total_balance: float, market_info: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _calculate_portfolio_metrics(
        ...
        # Calculate exposure
        exposure_ratio = (
            (total_balance - krw_balance) / total_balance if total_balance > 0 else 0
        )
        cash_ratio = (
            krw_balance / total_balance if total_balance > 0 else DEFAULT_CASH_RATIO
        )
    ...
    def _assess_drawdown_risk(
        self, portfolio: Dict[str, Any], recent_trades: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _calculate_drawdown_metrics(
        self, cumulative_values: List[float]
    ...
    def _assess_correlation_risk(
        ...
        return {
            ...
            "risk_level": (
                RISK_LEVEL_HIGH
                if max_correlation > self.risk_thresholds["correlation_high"]
                else RISK_LEVEL_LOW
            ),
    ...
    def _calculate_position_correlations(
        self, positions: List[str], market_data: Dict[str, Dict[str, Any]]
        ...
        for i, symbol1 in enumerate(positions):
            for j, symbol2 in enumerate(positions[i + 1 :], i + 1):
                correlation = self._estimate_correlation(symbol1, symbol2, market_data)
                correlations.append(correlation)

                if correlation > self.risk_thresholds["correlation_high"]:
                    correlated_pairs.append(
                        {"pair": f"{symbol1}-{symbol2}", "correlation": correlation}
                    )

        return correlations, correlated_pairs

    def _estimate_correlation(
        self, symbol1: str, symbol2: str, market_data: Dict[str, Dict[str, Any]]
    ) -> float:
    ...
    def _calculate_portfolio_volatility(
        ...
        return float(sum(weighted_volatilities))

    def _calculate_position_risk_score(
        self, position_ratio: float, profit_loss_pct: float, volatility: float
    ) -> float:
    ...
    def _assess_position_risk_factors(
        ...
        high_risk_positions = sum(
            1
            for p in position_risks.values()
    ...
    def _get_position_recommendations(
        ...
        for symbol, risk in position_risks.items():
            if risk["risk_level"] == RISK_LEVEL_CRITICAL:
                ...
                risk["risk_level"] == RISK_LEVEL_HIGH
                and risk["profit_loss_pct"] < HIGH_LOSS_THRESHOLD
            ):
    ...
    def _get_ai_risk_assessment(
        ...
        try:
            ...
            result = self.openai_client.analyze_with_prompt(
                prompt=prompt, system_message=system_message, temperature=AI_TEMPERATURE
            )

            return str(result.get("response", "Risk assessment unavailable"))
    ...
    def _create_ai_prompt(
        self, portfolio: Dict[str, Any], overall_risk: str, positions: List[str]
    ) -> str:
