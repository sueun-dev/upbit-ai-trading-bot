...
class AdaptiveRiskManager:
    ...
    def calculate_optimal_position_size(
        ...
        try:
            ...
            volatility_adjusted = self._adjust_for_volatility(
                kelly_adjusted, market_data
            )
            heat_adjusted = self._adjust_for_portfolio_heat(
                volatility_adjusted, portfolio
            )
            ai_adjusted = heat_adjusted * (
                1.0 if ai_approved else AI_REJECTION_MULTIPLIER
            )
    ...
    def _adjust_for_portfolio_heat(
        self, size: float, portfolio: Dict[str, Any]
    ) -> float:
    ...
    def _convert_to_krw_amount(
        self, size_ratio: float, portfolio: Dict[str, Any]
    ) -> float:
        ...
        return float(max(MIN_ORDER_KRW, available_krw))
    ...
    def _update_portfolio_heat(self, portfolio: Dict[str, Any]) -> None:
        ...
        self.portfolio_heat = min(
            1.0, self.portfolio_heat * PORTFOLIO_HEAT_DECAY_RATE + position_heat
        )
    ...
    def _calculate_current_exposure(self, portfolio: Dict[str, Any]) -> float:
        ...
        return float(crypto_value / total_balance)
    ...
    def _evaluate_circuit_conditions(
        ...
        if (
            ...
            and performance["trade_count"] >= MIN_TRADES_FOR_STATISTICAL_SIGNIFICANCE
        ):
            triggered = True
    ...
    def _calculate_daily_loss(self, trades: List[Dict[str, Any]]) -> float:
        ...
        if start_of_day_value <= 0:
            ...
            logger.warning(
                "Unable to determine start of day portfolio value, using 1M KRW"
            )
        ...
        return abs(today_loss) / start_of_day_value if today_loss < 0 else 0.0

    def _get_start_of_day_portfolio_value(
        self, trades: List[Dict[str, Any]], target_date: date
    ) -> float:
            ...
            if isinstance(timestamp, str):
                try:
                    ...
                    if trade_date == target_date:
                        ...
                        return float(trade.get("portfolio_value_before", 0))
                except (ValueError, AttributeError):
                    pass

        # If no trades today, fall back to the latest recorded portfolio value.
        status = get_latest_portfolio_status()
        if status:
            return float(status.get("total_balance", 0))

        return 0.0

    def _generate_sizing_reasoning(
        self, confidence: float, market_data: Dict[str, Any], portfolio: Dict[str, Any]
    ) -> str:
    ...
    def _build_sizing_result(
        ...
        return {
            ...
            "volatility_adjustment": (
                volatility_adjusted / kelly_adjusted if kelly_adjusted > 0 else 1
            ),
            "heat_adjustment": (
                heat_adjusted / volatility_adjusted if volatility_adjusted > 0 else 1
            ),
            "ai_adjustment": ai_adjusted / heat_adjusted if heat_adjusted > 0 else 1,
            "reasoning": self._generate_sizing_reasoning(
                confidence, market_data, portfolio
            ),
        }
