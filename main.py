...
def _require_env(name: str, value: Optional[str]) -> str:
    """Return a required environment value or exit with a clear message."""
    if not value:
...
def run_main_trading_loop(orchestrator: TradingOrchestrator) -> None:
    ...
    while True:
        try:
            ...
            logger.info(
                "😴 Waiting %ds before next trading cycle...", CHECK_INTERVAL_SECONDS
            )
...
def main() -> None:
    """Set up the trading system and run the main loop (console-script entry point)."""
    setup_logging()
    ...
    for insight in insights:
        if (
            insight["type"] == "success_rate"
            and insight.get("metrics", {}).get("total_trades", 0) > 0
        ):
            m = insight["metrics"]
            logger.info(
                f"📈 {m['success_rate']}% success ({m['successful_trades']}/{m['total_trades']})"
            )
        elif (
            insight["type"] == "failure_analysis"
            and insight.get("metrics", {}).get("total_failures", 0) > 0
        ):
