...
class TestEnhancedMarketDataCollector(unittest.TestCase):
    ...
    def test_get_enhanced_market_data_success(
        self, mock_get_ohlcv, mock_get_current_price
    ):
    ...
    def test_get_price_data_comprehensive(self):
        ...
        with patch(
            "src.data.collectors.enhanced_market_data.pyupbit.get_ohlcv"
        ) as mock_get_ohlcv:
            ...
            for field in expected_fields:
                ...
                self.assertIsInstance(
                    result[field], (int, float, np.integer, np.floating)
                )
    ...
    def test_get_volatility_metrics(self):
        ...
        with patch(
            ...
            volatile_data = pd.DataFrame(
                {
                    "open": [
                        45000 + i * 100 + np.random.normal(0, 500) for i in range(35)
                    ],
                    "high": [
                        46000 + i * 100 + np.random.normal(0, 500) for i in range(35)
                    ],
                    "low": [
                        44000 + i * 100 + np.random.normal(0, 500) for i in range(35)
                    ],
                    "close": [
                        45500 + i * 100 + np.random.normal(0, 500) for i in range(35)
                    ],
                    "volume": [1000 + i * 10 for i in range(35)],
                },
                index=pd.date_range(end="2025-01-01", periods=35, freq="D"),
    ...
    def test_symbol_case_handling(self):
        ...
        with patch(
            "src.data.collectors.enhanced_market_data.pyupbit.get_current_price"
        ) as mock_price:
